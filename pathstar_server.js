...
var pool = mysql.createPool({
    host:"43.242.156.6", //数据库地址
    user:"root",      //数据库用户名
    password:"cswangjie.1",    //数据库密码
    port:3306,      //数据库端口
    database:"csdn",//库名
...
server.use(cors({
  //允许程序列表
  origin:["http://localhost:8080","http://127.0.0.1:8080","http://43.242.156.6:4000"],
  // origin:"*",
...
server.get('/v1/user',(req,res)=>{
  ...
  if(uid.length>1){
    ...
    // for(var i=0;i<uid.length;i++){
      var sql=`select headpic,username,uid from csdn_user where uid in (${uid})`;
